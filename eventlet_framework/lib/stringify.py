...
def _mapdict(f, d): return dict([(k, f(v)) for k, v in d.items()])
def _mapdict_key(f, d): return dict([(f(k), v) for k, v in d.items()])
def _mapdict_kv(f, d): return dict([(k, f(k, v)) for k, v in d.items()])
...
class StringifyMixin(object):
    ...
    def to_jsondict(self, encode_string=base64.b64encode):
        ...
        """
        dict_ = {}
        def encode(key, val): return self._encode_value(
            key, val, encode_string)
    ...
    def from_jsondict(cls, dict_, decode_string=base64.b64decode,
        ...
        =============== =====================================================
        """
        def decode(k, x): return cls._decode_value(k, x, decode_string,
                                                   **additional_args)
